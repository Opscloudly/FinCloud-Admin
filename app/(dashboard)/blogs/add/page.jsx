...
export default function BlogAddForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const {
          data: { status, data },
        } = await Axios.get("blog-categories");
        if (status) {
          setCategories(data);
        }
      } catch (e) {
  ...
  const parseFormData = () => {
    ...
    for (const item in formData) {
      ...
      if (arrayInput.includes(item)) {
        for (const array_item of formData[item])
          data.append(`${item}[]`, array_item);
        continue;
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const {
        data: { status, message },
      } = await Axios.post("blogs", form_data, {
