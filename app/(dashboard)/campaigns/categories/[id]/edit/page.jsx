...
export default function BlogCategoryAddForm({ params: { id } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const {
          data: { status, data },
        } = await Axios.get(`campaigns/categories/${id}/edit`);
        if (status) {
  ...
  const prepareFormData = (data) => {
    const formData__initializer = { ...formData };
    const serverKeys = Object.keys(data);
    for (const i in formData__initializer) {
      if (serverKeys.includes(i)) {
        formData__initializer[i] = data[i];
      }
    }
    setFormData(formData__initializer);
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const {
        data: { status, message },
      } = await Axios.post(`campaigns/categories/${id}`, form_data, {
