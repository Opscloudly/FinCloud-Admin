  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        } = await Axios.get(`campaigns?${searchParams}`);
        if (status) {
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                <th className="table-head-cell border-b rounded-lg">Funding</th>
                <th className="table-head-cell border-b rounded-lg">Status</th>
                <th className="table-head-cell border-b rounded-lg">Action</th>
              </tr>
            </thead>
            <tbody>
              <HocData loaded={loaded} hasData={!!pageData.length} cols={7}>
                {pageData?.map((item, i) => (
                  <tr key={i}>
                    <td className="table-cell border-b">{item.title[lang]}</td>
                    <td className="table-cell border-b">
                      <span className="text-Gray-600">{item.investment}</span>
                    </td>
                    <td className="table-cell border-b">
                      <span className="text-Gray-600">{item.investors}</span>
                    </td>
                    <td className="table-cell border-b">
                    ...
                    <td className="table-cell border-b">{item.funding}</td>
                    ...
                    <td className="table-cell border-b">
                      <div className="flex gap-1">
                        {item.status === "PENDING" && (
                          <Link
                            href={`/campaigns/${item.id}/review`}
                            className="w-10 h-10 flex items-center justify-center group hover:bg-main-60 transition-all duration-150 rounded-md tooltip"
                            data-tip="Review campaign"
                          >
