...
import CampaignApprovalPopup from "@/components/popups/CampaignApprovalPopup";
...
export default function CampaignAddForm() {
  ...
  const initInputAd = {
    campaign_risk: "",
    campaign_highlights_tr: "",
    campaign_highlights_en: "",
  };
  const [formData, setFormData] = useState(initInput);
  const [formDataAd, setFormDataAd] = useState(initInputAd);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const {
          data: { status, data },
        } = await Axios.get(`campaign-review-helper-data/${id}`);
        if (status) {
  ...
  const handleInputAd = ({ target: { name, value } }) => {
    setFormDataAd((prev) => {
      return { ...prev, [name]: value };
    });
  };
  ...
  const parseAdditionalInfo = (data) => {
    data.append("campaign_risk", formDataAd.campaign_risk);
    data.append("campaign_highlights[tr]", formDataAd.campaign_highlights_tr);
    data.append("campaign_highlights[en]", formDataAd.campaign_highlights_en);
    return data;
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      let form_data = parseFormData();
      form_data = parseAdditionalInfo(form_data);
      form_data.append("_method", "PUT");
      const {
        data: { status, message },
      } = await Axios.post(`campaigns/approve/${id}`, form_data, {
  ...
  const handleRejectCampaign = async () => {
    try {
      ...
      const {
        data: { status, message },
      } = await Axios.post(`campaigns/reject/${id}`, form_data);
      if (status) {
        dispatchNotification({
          status: true,
          msg: message,
  ...
  return (
    <HOC type="dashboard">
      <section ref={headingCardRef} className="flex flex-col gap-y-6">
                ...
                {categories?.map((item, i) => (
                  <div key={i}>
                    <label htmlFor="lang" className="flex items-center gap-x-1">
                      ...
                      />
                      <span className="text-black">{item.label[lang]}</span>
                    </label>
                  </div>
                ))}
              </div>
        ...
        <CampaignApprovalPopup
          isOpen={openPopup}
          onClose={handleClosePopup}
          formData={formDataAd}
          handleInput={handleInputAd}
