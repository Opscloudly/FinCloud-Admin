...
export default function BlogCategoryEditForm({ params: { id } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const {
          data: { status, data },
        } = await Axios.get(`blog/categories/${id}/edit`);
        if (status) {
  ...
  const populateFormFields = (data) => {
    const langItems = ["category"];
    const switchItems = ["status"];
    const editableData = {};
    for (const item in initInput) {
      ...
      if (switchItems.includes(item)) {
        editableData[item] = Boolean(data[item]);
        continue;
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const {
        data: { status, message },
      } = await Axios.post(`blog/categories/${id}`, form_data, {
