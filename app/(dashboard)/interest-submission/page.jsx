...
export default function Blogs() {
  ...
  const handleSearch = () => {
    ...
    router.push(`/interest-submission?${queryString}`);
  };
  ...
  const handleExport = () => {
    ...
    url += `export/submit-interest?${queryString}`;
  ...
  const handleRemoveItem = async (e) => {
    ...
    try {
      ...
      if (popup.isConfirmed) {
        ...
        const {
          data: { status, message },
        } = await Axios.post(`subscribe/submit-interest/${id}`, data);
        if (status) {
          Swal.fire({
            icon: "success",
      ...
      <section className="flex flex-col gap-y-6">
        ...
        <div className="border border-Gray-200 rounded-lg">
          <table className="w-full">
            ...
            <tbody>
              <HocData loaded={loaded} hasData={!!pageData.length} cols={4}>
                {pageData?.map((item, i) => (
                  <tr key={i}>
                    <td className="table-cell border-b">{item.email}</td>
                    <td className="table-cell border-b">
                      <Pill label={item.type} className={item.type_class} />
                    </td>
                    <td className="table-cell border-b">
